...
def make_filter_dict(df,indexCols,paramDict):
    ...
    if len(indexCols)>0 :
          ...
          number=1
          filterDict,toFilter,indexColsSelectBox=get_items_to_filter(indexColsSelectBox,filterDict,number) 
...
def get_items_to_filter(indexColsSelectBox,filterDict,number):
    """
    ...
    st.write(filterColumn, " before")   
    st.write(indexColsSelectBox)  
    st.write(filterColumn, " chosen filter element before")     
    filterColumn=st.selectbox(chooseColumnLabel,indexColsSelectBox,
                                    index=0,key="filterColumn"+str(number))
    st.write(filterColumn," chosen filter element after")    
    toFilter=False 
